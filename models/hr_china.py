...
class ZuluHREmployeeContract(models.Model):
    ...
    @api.multi
    def _get_holiday_daily_fee_info(self):
        for item in self:
            item.holiday_daily_fee_info = "The total rate of an employee when they work on holiday"
    ...
    weekend_daily_fee_info = fields.Text(compute=_get_weekend_daily_fee_info)
    holiday_fee_info = fields.Text(compute=_get_holiday_fee_info)
    holiday_daily_fee_info = fields.Text(compute=_get_holiday_daily_fee_info)
    ...
    def contract_templ_change(self):
        ...
        # self.with_context({'employee_id': employee_id}).benefits_id = benefits_lines
        # self.with_context({'employee_id': employee_id}).deductions_id = deductions_lines
        # self.with_context({'employee_id': employee_id}).working_time = working_time_lines
